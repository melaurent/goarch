...
func NewGARCH(power float64, omega float64, alpha, gamma, beta []float64) *GARCH {
	...
	residuals := make([]float64, 0, maxDelay)
...
func (m *GARCH) Sample(nIntervals int) []float64 {
	M := len(m.beta)
	fsigmas := make([]float64, M, M)
	for i := 0; i < M; i++ {
		fsigmas[i] = m.omega / (1.0 - m.persistence)
	}
	sigIdx := 0
	...
	for i := 0; i < N; i++ {
		residuals[i] = m.residuals[i+m.resIdx] * m.residuals[i+m.resIdx]
	}
	resIdx := 0
	...
	for i := 0; i < nIntervals; i++ {
		...
		err := rand.NormFloat64() * math.Sqrt(sigma2)
